...
def set_up_forces(oa, computeQ=False, submode=-1, contactParameterLocation=".", membrane_center=250*angstrom,z_m=4):
    # apply forces
    forces = [
        basicTerms.con_term(oa),
        basicTerms.chain_term(oa),
        basicTerms.chi_term(oa),
        basicTerms.excl_term(oa,periodic=True),
        basicTerms.rama_term(oa),
        basicTerms.rama_proline_term(oa),
        basicTerms.rama_ssweight_term(oa, k_rama_ssweight=2*8.368),
        contactTerms.contact_term(oa, periodic=True),
        # for membrane protein simulation use contact_term below.
        # contactTerms.contact_term(oa, z_dependent=True, membrane_center=250*angstrom, k_relative_mem=3, periodic=True,z_m=4),
        ...
        # membraneTerms.membrane_term(oa, k=1*kilocalorie_per_mole, membrane_center=250*angstrom,z_m=4),
        # membraneTerms.membrane_term(oa, k=1*kilocalorie_per_mole, membrane_center=250*angstrom,periodic=False,z_m=4),
        ...
        debyeHuckelTerms.debye_huckel_term(oa, chargeFile="charge.txt",periodic=True),
    ]
    if computeQ:
