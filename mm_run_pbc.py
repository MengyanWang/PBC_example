...
def run(args):
    ...
    # Set PBC size Rebekah added
    import simtk.unit as unit
    box_vectors = np.array([[50.0, 0, 0], [0, 50.0, 0], [0, 0, 50]]) * unit.nanometers
    oa.system.setDefaultPeriodicBoxVectors(*box_vectors)
    ...
    if args.includeLigands:
        ...
        additional_cmd = ""
    os.system(f"{sys.executable} mm_analyze_PBC_dh.py {args.protein} -t {os.path.join(toPath, 'movie.dcd')} --subMode {args.subMode} -f {args.forces} {analysis_fasta} {additional_cmd} -c {chain}")
